...
class QuantizerV8(nn.Module):
    def __init__(self, n_e, e_dim, beta):
        super(QuantizerV8, self).__init__()

        self.e_dim = e_dim
        self.n_e = n_e
        self.beta = torch.tensor(beta)  # 将 beta 初始化为张量
    ...
    def forward(self, z):
        ...
        # 确保 beta 是张量并且参与 clamp 的计算
        self.beta = torch.clamp(self.beta * (1.05 if perplexity < 2.0 else 0.95), 0.1, 2.0)
    ...
    def get_codebook_entry(self, indices):
        index_flattened = indices.view(-1)
        z_q = self.embedding(index_flattened)
        return z_q.view(indices.shape + (self.e_dim,)).contiguous()
